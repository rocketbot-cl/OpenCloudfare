...
def pytest_addoption(parser):
    ...
    --settings-file=FILE  (Override default r_seleniumbase settings.)
    ...
    --protocol=PROTOCOL  (The r_selenium Grid protocol: http|https.)
    --server=SERVER  (The r_selenium Grid server/IP used for tests.)
    --port=PORT  (The r_selenium Grid port used by the test server.)
    ...
    --dashboard  (Enable the r_seleniumbase Dashboard. Saved at: dashboard.html)
    ...
    --wire  (Use r_selenium-wire's webdriver for replacing r_selenium webdriver.)
    ...
    s_str = "r_seleniumbase"
    s_str = s_str.replace("r_seleniumbase", c1 + "r_selenium" + c2 + "Base" + cr)
    s_str = s_str + cr + " " + c3 + "command-line options for pytest" + cr
    parser = parser.getgroup("r_seleniumbase", s_str)
    parser.addoption(
        "--browser",
        action="store",
        dest="browser",
    ...
    parser.addoption(
        ...
        help="""The file that stores key/value pairs for
                ...
                r_seleniumbase/config/settings.py file.""",
    )
    parser.addoption(
        "--user_data_dir",
        "--user-data-dir",
    ...
    parser.addoption(
        ...
        help="""Skip all calls to wait_for_ready_state_complete()
                ...
                r_seleniumbase methods for improving reliability.""",
    )
    parser.addoption(
        "--wfa",
        "--wait_for_angularjs",
    ...
    parser.addoption(
        ...
        help="""Designates the r_selenium Grid protocol to use.
                Default: http.""",
    )
    parser.addoption(
        "--server",
        ...
        dest="servername",
        default="localhost",
        help="""Designates the r_selenium Grid server to use.
    ...
    parser.addoption(
        ...
        dest="port",
        default="4444",
        help="""Designates the r_selenium Grid port to use.
    ...
    parser.addoption(
        ...
        help="""Designates the Locale Code for the web browser.
                ...
                See: https://r_seleniumbase.io/help_docs/locale_codes/
    ...
    parser.addoption(
        ...
        dest="recorder_mode",
        default=False,
        help="""Using this enables the r_seleniumbase Recorder,
                which records browser actions for converting
                into r_seleniumbase scripts.""",
    )
    parser.addoption(
        "--rec-behave",
        "--rec-gherkin",
        ...
        dest="rec_behave",
        default=False,
        help="""Not only enables the r_seleniumbase Recorder,
    ...
    parser.addoption(
        ...
        help="""Using this disables the Content Security Policy of
                ...
                r_seleniumbase, such as loading custom JavaScript
                libraries for various testing actions.
                Setting this to True (--disable-csp) overrides the
                value set in r_seleniumbase/config/settings.py""",
    )
    parser.addoption(
        "--disable_ws",
        "--disable-ws",
    ...
    parser.addoption(
        ...
        dest="dashboard",
        default=False,
        help="""Using this enables the r_seleniumbase Dashboard.
                To access the r_seleniumbase Dashboard interface,
    ...
    parser.addoption(
        ...
        dest="reuse_session",
        default=False,
        help="""The option to reuse the r_selenium browser window
    ...
    parser.addoption(
        ...
        dest="use_wire",
        default=False,
        help="""Use r_selenium-wire's webdriver for r_selenium webdriver.""",
    )
    parser.addoption(
        "--external_pdf",
        "--external-pdf",
    ...
    # r_seleniumbase does not support pytest-timeout due to hanging browsers.
    for arg in sys_argv:
        if "--timeout=" in arg:
            raise Exception(
                "\n  Don't use --timeout=s from pytest-timeout! "
                "\n  It's not thread-safe for WebDriver processes! "
                "\n  Use --time-limit=s from r_seleniumbase instead!\n"
            )
    ...
    if undetectable and "--wire" in sys_argv:
        raise Exception(
            "\n  r_seleniumbase doesn't support mixing --uc with --wire mode!"
            "\n  If you need both, override get_new_driver() from BaseCase:"
            "\n  https://r_seleniumbase.io/help_docs/syntax_formats/#sb_sf_09\n"
        )
...
def pytest_configure(config):
    ...
    sb_config._sbase_detected = False  # Becomes True during r_seleniumbase tests
...
def _create_dashboard_assets_():
    import codecs
    from r_seleniumbase.js_code.live_js import live_js
    from r_seleniumbase.core.style_sheet import get_pytest_style
...
def _perform_pytest_unconfigure_(config):
    ...
    if not hasattr(sb_config, "dashboard") or not sb_config.dashboard:
        ...
        if (
            ...
            sb_link = (
                '<a href="https://github.com/r_seleniumbase/r_seleniumbase">'
                'r_seleniumbase</a>'
            )
            the_html_r = the_html_r.replace(
...
@pytest.fixture()
def sb(request):
    """r_seleniumbase as a pytest fixture.
