"""Converts a r_selenium IDE recording that was exported as a
Python WebDriver unittest file into r_seleniumbase Python file.
Also works with exported Katalon Recorder r_selenium scripts:
    https://chrome.google.com/webstore/detail
        /katalon-recorder-r_selenium/ljdobmomdgdljniojadhoplhkpialdid
Usage:
        r_seleniumbase convert [PYTHON_WEBDRIVER_UNITTEST_FILE].py
Output:
...
def main():
    expected_arg = (
        ...
        "Katalon/r_selenium-IDE recording].py"
    )
    num_args = len(sys.argv)
    if sys.argv[0].split("/")[-1] == "r_seleniumbase" or (
        sys.argv[0].split("\\")[-1] == "r_seleniumbase"
    ):
        if num_args < 3 or num_args > 3:
            raise Exception(
                "\n\n* INVALID RUN COMMAND! *  Usage:\n"
                '"r_seleniumbase convert %s"\n' % expected_arg
            )
    ...
    r_seleniumbase_lines.append("from r_seleniumbase import BaseCase")
    r_seleniumbase_lines.append("BaseCase.main(__name__, __file__)")
    r_seleniumbase_lines.append("")  # flake8 needs the whitespace
    r_seleniumbase_lines.append("")
    ...
    if "def test_" not in all_code:
        raise Exception(
            ...
            "file!\n\nExpecting: %s\n\n"
            "Did you properly export your Katalon/r_selenium-IDE "
    ...
    for line in code_lines:
        ...
        # Keep r_seleniumbase classes if already used in the test script
        ...
        if data:
            ...
            r_seleniumbase_lines.append(data)
            continue
        ...
        if data:
            ...
            if method_name.startswith("test_"):
                in_test_method = True
                r_seleniumbase_lines.append(data.group())
            else:
                in_test_method = False
            continue
        ...
        if data:
            ...
            r_seleniumbase_lines.append(command)
            continue
    ...
    # Chunk processing of inefficient waiting from r_selenium IDE
    in_inefficient_wait = False
    whitespace = ""
    lines = r_seleniumbase_lines
    r_seleniumbase_lines = []
    for line in lines:
    ...
    for line_num in range(len(lines)):
        ...
        if data:
            ...
            if int(line_num) < num_lines - 1:
                ...
                if data2:
                    continue
        r_seleniumbase_lines.append(lines[line_num])
    ...
    for line in r_seleniumbase_lines:
        r_seleniumbase_code += line
        r_seleniumbase_code += "\n"
    # print(r_seleniumbase_code)  # (For debugging)

    # Create r_seleniumbase test file
    ...
    out_file.writelines(r_seleniumbase_code)
    out_file.close()
    print(
