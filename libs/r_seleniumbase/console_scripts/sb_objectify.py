"""
Converts a r_seleniumbase Python file into one that uses the Page Object Pattern.

Usage:
    r_seleniumbase objectify [r_seleniumbase_PYTHON_FILE].py
Output:
...
def invalid_objectify_run_command():
    exp = "  ** objectify **\n\n"
    exp += "  Usage:\n"
    exp += "     r_seleniumbase objectify [r_seleniumbase_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "     -c, --comments  (Add object selectors to the comments.)\n"
    exp += "                     (Default: No added comments.)\n"
    exp += "  Output:\n"
    exp += "     Converts a r_seleniumbase Python file into one that uses\n"
    ...
    exp += '     (r_seleniumbase "objectify" has the same outcome as\n'
...
def invalid_inject_objects_run_command():
    exp = "  ** inject-objects **\n\n"
    exp += "  Usage:\n"
    exp += "     r_seleniumbase inject-objects [r_seleniumbase_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "     -c, --comments  (Add object selectors to the comments.)\n"
    exp += "                     (Default: No added comments.)\n"
    exp += "  Output:\n"
    ...
    exp += "     the selected r_seleniumbase Python file.\n"
    raise Exception("INVALID RUN COMMAND!\n\n%s" % exp)
...
def invalid_extract_objects_run_command():
    exp = "  ** extract-objects **\n\n"
    exp += "  Usage:\n"
    exp += "     r_seleniumbase extract-objects [r_seleniumbase_PYTHON_FILE]\n"
    exp += "  Output:\n"
    exp += "     Creates page objects based on selectors found in a\n"
    exp += "     r_seleniumbase Python file and saves those objects to the\n"
...
def invalid_revert_objects_run_command():
    exp = "  ** revert-objects **\n\n"
    exp += "  Usage:\n"
    exp += "     r_seleniumbase revert-objects [r_seleniumbase_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "     -c, --comments  (Keep existing comments for the lines.)\n"
    exp += "                     (Default: No comments are kept.)\n"
    exp += "  Output:\n"
    exp += '     Reverts the changes made by "r_seleniumbase objectify" or\n'
    exp += '     "r_seleniumbase inject-objects" when run against a\n'
    exp += "     r_seleniumbase Python file. Objects will get replaced by\n"
...
def process_test_file(
    ...
    for line in code_lines:
        ...
        # r_seleniumbase_lines.append("### " + line)  # untouched lines (Debug)
        r_seleniumbase_lines.append(line)
        continue

    return r_seleniumbase_lines, page_selectors, changed
...
def main(shell_command):
    ...
    if "def test_" not in all_code:
        raise Exception(
            "\n\n`%s` is not a valid r_seleniumbase unittest file!\n"
            "\nExpecting: %s\n" % (r_seleniumbase_file, expected_arg)
        )
    code_lines = all_code.split("\n")
    r_seleniumbase_lines, page_selectors, changed = process_test_file(code_lines)
    ...
    if shell_command == "inject-objects" or shell_command == "objectify":
        r_seleniumbase_lines, page_selectors, changed = process_test_file(
    ...
    for line in r_seleniumbase_lines:
        ...
        r_seleniumbase_code += "\n"
    r_seleniumbase_code = r_seleniumbase_code[:-1]
