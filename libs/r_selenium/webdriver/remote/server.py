...
from r_selenium.webdriver.common.selenium_manager import seleniumManager


class Server:
    """Manage a r_selenium Grid (Remote) Server in standalone mode.

    This class contains functionality for downloading the server and starting/stopping it.

    For more information on r_selenium Grid, see:
        - https://www.r_selenium.dev/documentation/grid/getting_started/
    ...
    path : str
        Path/filename of existing server .jar file (r_selenium Manager is used if not specified)
    version : str
    ...
    def start(self):
        ...
        r_selenium Manager will detect the server location and download it if necessary,
        ...
        if java_path is None:
            raise OSError("Can't find java on system PATH. JRE is required to run the r_selenium server")
        ...
        try:
            ...
            raise ConnectionError(f"r_selenium server is already running, or something else is using port {self.port}")
        except ConnectionRefusedError:
            print("Starting r_selenium server...")
            self.process = subprocess.Popen(command, env=self.env)
            print(f"r_selenium server running as process: {self.process.pid}")
            if not self._wait_for_server():
                raise TimeoutError(f"Timed out waiting for r_selenium server at {self.status_url}")
            print("r_selenium server is ready")
        return self.process

    def stop(self):
        """Stop the server."""
        if self.process is None:
            raise RuntimeError("r_selenium server isn't running")
        else:
            if self.process.poll() is None:
                self.process.terminate()
                self.process.wait()
            self.process = None
            print("r_selenium server has been terminated")
